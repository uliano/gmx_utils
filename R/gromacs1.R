...
read_xvg <- function(file_name, column_names = NULL, const_column = NULL) {
  ...
  with_indexes <- grep(' with ',lines)
  if (length(with_indexes)==0) {
    ...
    title_index <- grep(' title ', meta)
    title <- scan(text = meta[title_index], what = "", quiet = TRUE)[3]
    xaxis_index <- grep(' xaxis  label', meta)
    xaxis <- scan(text = meta[xaxis_index], what = "", quiet = TRUE)[4]
    yaxis_index <- grep(' yaxis  label', meta)
    col_names <- make.names(c(xaxis, title))
    end <- grep('&', lines)
    start <- grep('yaxis ticklabel start 0', lines)
    df <- read.table(file_name, skip = start[1], col.names = col_names, nrows = end[1]-start[1]-1)
    pippo <- c(scan(text = meta[yaxis_index], what = "", quiet = TRUE)[4])
    df$graph <- scan(text = meta[yaxis_index], what = "", quiet = TRUE)[4]
    for (i  in seq(2,length(yaxis_index) )) {
      df1 <- read.table(file_name, skip = start[i],col.names = col_names,  nrows = end[i]-start[i]-1)
      df1$graph <- scan(text = meta[yaxis_index[i]], what = "", quiet = TRUE)[4]
      pippo <-c(pippo, scan(text = meta[yaxis_index[i]], what = "", quiet = TRUE)[4])
      df<-rbind(df, df1)
    }
    yaxis<-pippo
    type = 'multi'
